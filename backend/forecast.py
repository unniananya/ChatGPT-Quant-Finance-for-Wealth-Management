...
def get_stock_data(stock_symbol, steps):
    ...
    # if len(stock_data) == 0:
        # raise Error(f"Failed to download stock price data for symbol {stock_symbol} from yfinance!")
...
def get_daily_news(symbol, finnhub_client):
    ...
    try:
        latest_news = finnhub_client.company_news(symbol, _from=start_date, to=end_date)
        if len(latest_news) == 0:
            return []  # No news found

        formatted_news = [
            {
                "date": datetime.fromtimestamp(n['datetime']).strftime('%Y-%m-%d %I:%M:%S %p'),
                "headline": n['headline'],
                "summary": n['summary'],
                "url": n['url'],
            } for n in latest_news
        ]
        formatted_news.sort(key=lambda x: x['date'], reverse=True)  # Sorting by date, newest first
...
def get_latest_news(symbol, finnhub_client):
    current_time = datetime.now()
    start_time = current_time - timedelta(hours=12)  # Look back 1 hour; adjust as needed
    start_date = start_time.strftime('%Y-%m-%d')
    end_date = current_time.strftime('%Y-%m-%d')
    ...
    try:
        latest_news = finnhub_client.company_news(symbol, _from=start_date, to=end_date)
        if len(latest_news) == 0:
            # Handle no news found case here, perhaps just return an empty list
            return []
...
def predict(ticker, n_weeks, use_basics):
...
# date = get_curday()
# info, prompt = construct_prompt("AAPL",date, 3, False)
# print("info:", info)
# print("############\n")
# print("prompt:", prompt)
# predict("AAPL",date, 3, False )